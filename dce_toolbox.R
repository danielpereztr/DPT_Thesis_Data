...
alts <- 2
nochoice <- TRUE
priors <- c(0,1.654,0.547,0.25, -1.5)
alg <- "cea"
sigma <- diag(length(priors))
rep <- 100
priors <- rmvnorm(100, priors, sigma)
...
dce_toolbox <- function(attributes, csets, alts, nochoice, priors, alg){
  ...
  dummify <- function(randomdce, alts){
    ...
    for (col in 1:(ncol(randomdce[,2:ncol(randomdce)]))){
      temp_dum <- dummy_cols(randomdce[,col+1])
    ...
    temp_mat <- add_column(temp_mat, rep(1:alts, length.out = nrow(temp_mat)) ,.after = "task")
    colnames(temp_mat)[2] <- "alt"
    temp_mat <- temp_mat[, -which(stri_sub(colnames(temp_mat),-1) == 1)]
  ...
  D.error <- function(des, priors){
    ...
    des <- merge(des, aggregate(expv ~ task, FUN = "sum", data = des), by = "task")
  ...
  cea <- function(randomdce, attributes, alts, csets, priors, nochoice){
    ...
    for (it in 1:10){
        ...
        if (nochoice == FALSE | (nochoice == TRUE && row %% alt)){
          for (at in 1:(ncol(randomdce)-1)){ #attribute
            for (lev in 1:attributes[at]){
              ...
              if (nochoice == TRUE){
                temp_mat <- add_column(temp_mat, "optout" = rep(append(rep(0, alts), 1), csets), .after = 2)
              }
              min <- append(min, DBerr(priors, as.matrix(temp_mat[,-c(1,2)]), alts))
            }
      ...
      it_d <- append(it_d, DBerr(priors, as.matrix(temp_mat[,-c(1,2)]), alts))
      change <- (it_d[(length(it_d))] - it_d[(length(it_d)-1)])/it_d[(length(it_d)-1)]
      change <- change*100
      ...
      if (change >= -0.1 && it >= 2){
        ...
        cat("\nIteration ", it, ": DB-error ", DBerr(priors, as.matrix(temp_mat[,-c(1,2)]), alts))
        cat("\nChange: ", change, "% \n")
      }
    }
    Derror <- DBerr(priors, as.matrix(temp_mat[,-c(1,2)]), alts)
